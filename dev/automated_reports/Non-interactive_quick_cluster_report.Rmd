...
} else if(params$cluster_id %in% c(96, 113, 121, 127, 189, 226, 240, 260, 286, 295, 324, 334, 374,
                                   361)){
...
} else if(params$cluster_id %in% c(141, 375)){
...
plot_snp_matrix <- function(path_to_res, minicl, all_samples = FALSE){
  ...
  snps_df <- data.table(read.table(path_to_snps, sep = "\t", header = TRUE, row.names = 1)) %>%
    select(-Reference) # Removing Reference Colum
  ...
  setnames(snps_df, old = colnames(snps_df), new = reports$glims[match(colnames(snps_df),
                                                                    names(snps_df))])
  ...
  ht <- Heatmap(snps_matrix,
                ...
                cell_fun = function(j, i, x, y, width, height, fill) {
                  ...
                  if (i == j) {
                    ...
                    if (!is.na(val)) {
                      grid.text(label = val, x = x, y = y, gp = gpar(col = "black", fontsize = 8))
                    }
                  }
                },
                width = unit(10, "cm"),    
