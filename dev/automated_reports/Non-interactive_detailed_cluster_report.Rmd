...
<!--
*******************************************************************
Title: Non-interactive_detailed_cluster_report.Rmd
Description: Report at cluster level, indicating species, STs and antimicrobial genes globally present.
...
for(s in species){
  ...
  amr_report <- combined_amr_report[`Type` == "AMR" & genome %in% samples]
  cat("### ARGs chromosomiques \n\n")
  # Loading only chromosomal ARGs
  chr_arg <- amr_report[molecule_type == "chromosome"]
  # Arranging AMR results
  out <- chr_arg %>%
        select(genome, `Element symbol`, `Element name`, Class, Subclass) %>%
        distinct() %>%
        group_by(`Element symbol`, `Element name`, Class, Subclass) %>%
...
```
```{r cleaning}
rm(global_theme, params, reports, mobtyper_reports)
```
