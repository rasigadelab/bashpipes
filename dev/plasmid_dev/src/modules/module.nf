...
process align_to_reference {
  ...
  output:
    tuple val(resistance), val(inc), val(sample), emit : plasmid_seq
    path("analyses/$resistance/$inc/quast_results*/*")


  script:
  """
  TIMESTAMP=\$(date +%Y%m%d_%H%M%S)
  OUT_DIR=analyses/$resistance/$inc/quast_results_\${TIMESTAMP}
  ...
  then
    ...
    quast -r \$REFERENCE.fasta -g \$REFERENCE.gff3 --circos -o \${OUT_DIR} \$SAMPLES_FASTA
  elif [ $resistance = "blaVIM-4" -a $inc = "IncHI2A" ]
  then
