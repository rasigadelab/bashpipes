# Title: 300_bhre.R
# Description: Outputs an Excel file, checking genome assemblies status after QC analysis. 
...
# CHECK QC file for QC_PASS + Sourmash species + MLST species
qc_df <- data.table(read.table("./genomes/assembly_summary_illumina_only.tsv", header = TRUE, sep = "\t"))
qc_df <- qc_df[,c("genome", "MLST_species", "sourmash_species", "NEW_QC_illumina_pass", "contaminated")]
colnames(qc_df) <- c("id", "MLST_species", "SOURMASH_species", "QC_illumina_pass", "contaminated")

# Merge metadata + qc info
...
enterobacter <- c("ecloacae", "ecomplex", "ehormaechei", "espp.", "easburiae", "eroggenkampii",
                  "ekobei", "equasihormaechei", "ehormaechei_a", "emarmotae", "edissolvens")
ecoli <- c("ecoli_achtman_4", "escherichia")
...
pputida <- c("pperadeniyensis")
pacnes <- c("pacnes_3", "cacnes")
...
tmp2 <- redef_species(pputida, "pputida")
tmp2 <- redef_species(pacnes, "pacnes")
...
for(sample in tmp2$SAMPLE_ID){
  ...
  if(current$QC_illumina_pass==FALSE & current$contaminated=="FALSE"){
  ...
  if(current$QC_illumina_pass==TRUE & current$contaminated=="FALSE"){
...
# Output
qc_df <- data.table(read.table("./genomes/assembly_summary_illumina_only.tsv", header = TRUE, sep = "\t"))
qc_df <- qc_df[qc_df$genome %in% tmp3$SAMPLE_ID,]
