/*
 * Module collection: Bacteria Plasmid Comparison Pipeline
 *
 * Description:
...
process align_to_reference {
  ...
  then
    REFERENCE=${params.align_to_reference["reference_oxa48_incLM"]}
    quast.py -r \$REFERENCE.fasta -g \$REFERENCE.gff3 --circos -o \${OUT_DIR} \$SAMPLES_FASTA
  elif [ $resistance = "blaVIM-1" -a $inc = "IncN" ]
  then
    REFERENCE=${params.align_to_reference["reference_vim1_incN"]}
    quast.py -r \$REFERENCE.fasta -g \$REFERENCE.gff3 --circos -o \${OUT_DIR} \$SAMPLES_FASTA
  elif [ $resistance = "blaVIM-1" -a $inc = "IncL-M" ]
  then
    REFERENCE=${params.align_to_reference["reference_vim1_incLM"]}
    quast.py -r \$REFERENCE.fasta -g \$REFERENCE.gff3 --circos -o \${OUT_DIR} \$SAMPLES_FASTA
  elif [ $resistance = "blaVIM-4" -a $inc = "IncHI2A,rep_cluster_1088" ]
  then
    REFERENCE=${params.align_to_reference["reference_vim4_incHI2A"]}
    quast.py -r \$REFERENCE.fasta -g \$REFERENCE.gff3 --circos -o \${OUT_DIR} \$SAMPLES_FASTA
  elif [ $resistance = "blaOXA-48" -a $inc = "IncC" ]
  then
    REFERENCE=${params.align_to_reference["reference_oxa48_incC"]}
    quast.py -r \$REFERENCE.fasta -g \$REFERENCE.gff3 --circos -o \${OUT_DIR} \$SAMPLES_FASTA
  elif [ $resistance = "blaOXA-48" -a $inc = "IncL-M,IncR" ]
  then
    REFERENCE=${params.align_to_reference["reference_oxa48_incLMR"]}
    quast.py -r \$REFERENCE.fasta -g \$REFERENCE.gff3 --circos -o \${OUT_DIR} \$SAMPLES_FASTA
  elif [ $resistance = "blaOXA-48" -a $inc = "-" ]
  then
    REFERENCE=${params.align_to_reference["reference_oxa48_noInc"]}
    quast.py -r \$REFERENCE.fasta -g \$REFERENCE.gff3 --circos -o \${OUT_DIR} \$SAMPLES_FASTA
  elif [ $resistance = "blaNDM-1" -a $inc = "IncN" ]
  then
    REFERENCE=${params.align_to_reference["reference_ndm1_incN"]}
    quast.py -r \$REFERENCE.fasta -g \$REFERENCE.gff3 --circos -o \${OUT_DIR} \$SAMPLES_FASTA
  elif [ $resistance = "blaNDM-1" -a $inc = "IncFIB" ]
  then
    REFERENCE=${params.align_to_reference["reference_ndm1_incFIB"]}
    quast.py -r \$REFERENCE.fasta -g \$REFERENCE.gff3 --circos -o \${OUT_DIR} \$SAMPLES_FASTA
  elif [ $resistance = "blaNDM-1" -a $inc = "IncHI1A" ]
  then
    REFERENCE=${params.align_to_reference["reference_ndm1_incHI1A"]}
    quast.py -r \$REFERENCE.fasta -g \$REFERENCE.gff3 --circos -o \${OUT_DIR} \$SAMPLES_FASTA
  elif [ $resistance = "blaNDM-1" -a $inc = "IncC" ]
  then
    REFERENCE=${params.align_to_reference["reference_ndm1_incC"]}
    quast.py -r \$REFERENCE.fasta -g \$REFERENCE.gff3 --circos -o \${OUT_DIR} \$SAMPLES_FASTA
  elif [ $resistance = "blaNDM-1" -a $inc = "-" ]
  then
    REFERENCE=${params.align_to_reference["reference_ndm1_noInc"]}
    quast.py -r \$REFERENCE.fasta -g \$REFERENCE.gff3 --circos -o \${OUT_DIR} \$SAMPLES_FASTA
  elif [ $resistance = "blaNDM-1" -a $inc = "IncC,IncN,IncR" ]
  then
    REFERENCE=${params.align_to_reference["reference_ndm1_incCNR"]}
    quast.py -r \$REFERENCE.fasta -g \$REFERENCE.gff3 --circos -o \${OUT_DIR} \$SAMPLES_FASTA
  else
