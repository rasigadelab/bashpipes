...
subtitle: "GH Nord, Services de Réanimation-Soins intensifs-Néonatologie UF24170/24171/24172, *S. marcescens* (`r params$replicon`)"
...
params:
  ...
  replicon: "Chromosome"
  env_samples: ""
  path_to_reports: "../Script_Tests/2023-11-23_clone_reports.Rdata"
...
Ce rapport d'études de cluster étudie les isolats de l'espèce ___S. marcescens___ des services de réanimation, soins intensifs et néonatologie du GH Nord.
...
if(params$species == "chromosome" & params$cluster_id == 3){
...
out <- reports %>%
  select(genome, mlst_species, sampling_date, ipp) %>%
...
g <- ggplot(data=out, aes(x=month, fill = patients)) +
   ...
   ggtitle(paste("Histogramme des dates de prélèvement \nde l'espèce", species_in_title)) +
   labs(x = "Date de prélèvement par mois", y = "Nombre d'échantillons", fill = "Type de souches prélevées") +
  scale_x_date(date_breaks = "month", date_labels = "%m/%Y", date_minor_breaks = "month", 
               limits = c(as.Date("2022-01-01"), as.Date("2023-01-01"))) +
...
rm(first_month, last_month, out, g, earliest, latest, nb_samples, nb_patients)

# In case you need to add limits to the graph because everything is located on 1 month:
# scale_x_date(date_breaks = "month", date_labels = "%m/%Y", date_minor_breaks = "month", 
#                limits = c(as.Date("2023-01-01"), as.Date("2023-06-01"))) +
...
if(params$cluster_id=="2"){
  rectal <- c("022119761201-03", "022153065501-02", "022178399701-03", "022098072101-02",
              "022106460701-01", "022113256501-01", "022152814701-03", "022165829801-02",
              "022161261701-01", "022178953001-01", "022081308401-01")
  liquide_drain <- c("022150003401-01")

  tab <- tab %>%
    mutate(type_prlvt=ifelse(ipp=="Prélèvement d'environnement", "Ecouvillon", "")) %>% 
    mutate(type_prlvt=ifelse(glims %in% rectal, "Dépistage rectal", type_prlvt)) %>%
    mutate(type_prlvt=ifelse(glims %in% liquide_drain, "Liquide de drain", type_prlvt)) %>%
    relocate(type_prlvt, .before =3)
  
  # Outputting table 
  kable(tab, col.names = c("GLIMS", "Identifiant Patient (IPP)", "Type de prélèvement", "Date de prélèvement", "Espèce MALDI-ToF", "MLST Sequence Type", "Cluster"),
        align = "lllcc", "latex", booktabs = TRUE) %>%
    kable_styling(latex_options = c("striped", "scale_down", "HOLD_position", "repeat_header")) %>%
    column_spec(3, width = "8em")
} else {
  # Outputting table 
  kable(tab, col.names = c("GLIMS", "Identifiant Patient (IPP)", "Date de prélèvement", "Espèce MALDI-ToF", "MLST Sequence Type", "Cluster"),
        align = "lllcc", "latex", booktabs = TRUE) %>%
    kable_styling(latex_options = c("striped", "scale_down", "HOLD_position", "repeat_header"))
}
...
```{r cleaning}
rm(global_theme, params, reports, species, dist_plot, usePackage, nb_snps, clust_dist)
```
